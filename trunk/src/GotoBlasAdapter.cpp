...
void GotoBlasAdapter::dgemm(const enum CBLAS_ORDER_ Order, const enum CBLAS_TRANSPOSE_ TransA,
...
void GotoBlasAdapter::dscal(const int N, const double alpha, double *X, const int incX) const {
	api.cblas_dscal(N, alpha, X, incX);
}

void GotoBlasAdapter::daxpy(const int N, const double alpha, const double *X,
		                    const int incX, double *Y, const int incY) const {
	api.cblas_daxpy(N, alpha, (double *) X,
				incX, Y, incY);
}

int GotoBlasAdapter::dgetrf(int *m, int *n, double *a,
