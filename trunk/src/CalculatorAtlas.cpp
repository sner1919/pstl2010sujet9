...
template <class T>
void CalculatorAtlas<T>::mult(IMatrix<T>& Res, IMatrix<T>& A, IMatrix<T>& B){
	if(!(A.getN() == B.getM()
					 && Res.getM() == A.getM() && Res.getN() == B.getN())) throw domain_error("produit impossible");
	cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
		A.getM(), B.getN(), A.getN(),
		1.0, A.getData(), A.getM(),
		B.getData(), B.getM(),
		0.0, Res.getData(), Res.getM());
}

template <class T>
void CalculatorAtlas<T>::mult(IMatrix<T>& Res,IMatrix<T>& A, T x){
	// Vérifications
...
template <class T>
void CalculatorAtlas<T>::add(IMatrix<T>& Res,IMatrix<T>& A, IMatrix<T>& B){
	//dgeadd_(A.getData(),A.getM(),CblasNoTrans,B.getData(),B.getM(),
	//		CblasNoTrans,Res.getData(),Res.getM(),Res.getM(),Res.getN());
}

template <class T>
void CalculatorAtlas<T>::transpose(IMatrix<T>& Res, IMatrix<T>& A){
	// Vérifications
...
template <class T>
void CalculatorAtlas<T>::LU(IMatrix<T>& L, IMatrix<T>& U, IMatrix<T>& A){
	// Vérifications
	if(!(A.getM() == A.getN()
...
template class CalculatorAtlas<double>;
