...
void Processor<T>::LU(IMatrix<T>& L, IMatrix<T>& U, const IMatrix<T>& A) const {
	...
	pthread_t th1 = generator.generateError(Lc, 1, 1, Lc.getM(), 1, Lc.getN());
	pthread_t th2 = generator.generateError(Ur, 1, 1, Ur.getM(), 1, Ur.getN());
	calculator.LU(Lc, Ur, Af);
	pthread_join(th1, NULL);
	pthread_join(th2, NULL);
	while(Lc.columnErrorDetection() || Ur.rowErrorDetection()) calculator.LU(Lc, Ur, Af);
}
